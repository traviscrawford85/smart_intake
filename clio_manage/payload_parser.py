...
class PayloadParser:
    ...
    def detect_payload_type(payload: Dict[str, Any]) -> str:
        ...
        """
        logger.info("Detecting payload type", extra={"payload_keys": list(payload.keys())})
    ...
    def parse_direct_payload(payload: Dict[str, Any]) -> BotDataInput:
        ...
        try:
            ...
            logger.info("Direct payload parsed successfully", extra={
        ...
        try:
            ...
            for i, lead in enumerate(envelope.inbox_leads):
                try:
                    ...
                    mapped_data = PayloadParser._map_envelope_lead_to_bot_data(lead, envelope)
                    bot_data = BotDataInput(**mapped_data)
                    parsed_leads.append(bot_data)
                    
                    logger.info(f"Envelope lead {i+1} parsed successfully", extra={
                        "lead_name": f"{mapped_data['first_name']} {mapped_data['last_name']}",
                        "source": mapped_data['source']
                    })
                    
                except Exception as e:
                    logger.error(f"Failed to parse envelope lead {i+1}", extra={"error": str(e)})
                    continue
    ...
    def parse_mixed_payload(payload: Dict[str, Any]) -> BotDataInput:
        ...
        try:
            ...
            mapped_data = {
                ...
                "message": PayloadParser._extract_message(unified.message) or "Voice agent intake submission",
                "email": unified.email,
            ...
            logger.info("Mixed payload parsed successfully", extra={
                "lead_name": f"{mapped_data['first_name']} {mapped_data['last_name']}",
                "source": mapped_data['source']
            })
    ...
    @staticmethod
    def _map_envelope_lead_to_bot_data(lead: CaptureNowInboxLead, envelope: CaptureNowEnvelope) -> Dict[str, Any]:
        """
...
def parse_incoming_payload(payload: Dict[str, Any]) -> Union[BotDataInput, List[BotDataInput]]:
    """
    ...
    """
    logger.info("Starting payload parsing", extra={
    ...
    try:
        if payload_type == "direct":
            ...
            logger.error("Cannot parse unknown payload format", extra={"payload": payload})
            raise ValueError(f"Unknown payload format. Keys: {list(payload.keys())}")
    
    except Exception as e:
        logger.error("Payload parsing failed", extra={"error": str(e), "payload_type": payload_type})
        raise
...
def normalize_to_clio_format(bot_data: BotDataInput) -> ClioInboxLead:
    ...
    """
    logger.info("Normalizing to Clio format", extra={
