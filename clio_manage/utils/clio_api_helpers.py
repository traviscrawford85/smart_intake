...
class PaginationInfo:
    ...
    def from_response_headers(cls, headers: Dict[str, str]) -> 'PaginationInfo':
        ...
        return cls(
            ...
            total_count=int(headers.get('X-Total-Count', 0)) if headers.get('X-Total-Count') else None,
            total_pages=int(headers.get('X-Total-Pages', 1)) if headers.get('X-Total-Pages') else None,
...
class ClioAPIHelper:
    ...
    def __init__(self, max_requests: int = 100, window_seconds: int = 60, per_page: int = 50):
    ...
    async def get_all_contacts(self, client: httpx.AsyncClient) -> List[Dict[str, Any]]:
        ...
        async for contacts, pagination in self.paginator.paginate_all(client, url):
            ...
            print(f"Retrieved {len(contacts)} contacts (page {pagination.current_page})")
        
        return all_contacts
    
    async def get_all_custom_actions(self, client: httpx.AsyncClient) -> List[Dict[str, Any]]:
        ...
        async for actions, pagination in self.paginator.paginate_all(client, url):
            ...
            print(f"Retrieved {len(actions)} custom actions (page {pagination.current_page})")
        
        return all_actions
    
    async def get_all_webhook_subscriptions(self, client: httpx.AsyncClient) -> List[Dict[str, Any]]:
        ...
        async for subscriptions, pagination in self.paginator.paginate_all(client, url):
            ...
            print(f"Retrieved {len(subscriptions)} webhook subscriptions (page {pagination.current_page})")
